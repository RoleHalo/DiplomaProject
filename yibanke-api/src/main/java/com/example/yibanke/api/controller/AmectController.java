...
import com.example.yibanke.api.websocket.WebSocketService;
...
@Tag(name = "AmectController", description = "缴费Web接口")
@Slf4j
public class AmectController {
    ...
    public R insert(@Valid @RequestBody InsertAmectForm form) {
        ...
        for (Integer userId : form.getUserId()) {
            ...
            amect.setTypeId(form.getTypeId().intValue());
    ...
    @Operation(summary = "创建Native支付缴费订单")
    @SaCheckLogin
    ...
    public void recieveMessage(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        //利用数字证书验证收到的响应内容，避免伪造付款结果发送给Web方法。
        //验证
        if (WXPayUtil.isSignatureValid(xml, key)) {
            ...
            if ("SUCCESS".equals(resultCode) && "SUCCESS".equals(returnCode)) {
                String outTradeNo = map.get("out_trade_no");    //缴费单UUID
                //更新订单状态
                HashMap param = new HashMap() {{
                    // 2状态 为 已支付
                    put("status", 2);
                ...
                if (rows == 1) {
                    ...
                    System.out.println("缴费用户userId!!!!!!!!!!!!!!!!!!!!!!!!!!"+userId);
                    //向用户推送结果
    ...
    @Operation(summary = "查询Native支付缴费订单结果")
    @SaCheckLogin
