...
import com.example.yibanke.api.db.dao.TbAmectMapper;
import com.example.yibanke.api.db.pojo.TbAmect;
import com.example.yibanke.api.exception.YibankeException;
import com.example.yibanke.api.service.TbAmectService;
import com.example.yibanke.api.wxpay.MyWXPayConfig;
import com.example.yibanke.api.wxpay.WXPay;
import com.example.yibanke.api.wxpay.WXPayUtil;
...
@Service
@Slf4j
public class TbAmectServiceImpl implements TbAmectService {

    @Autowired
    private TbAmectMapper amectDao;
    ...
    public String createNativeAmectPayOrder(HashMap param) {
        ...
        if(map!=null && map.size()>0){
            String amount = new BigDecimal(MapUtil.getStr(map,"amount")).multiply(
                    new BigDecimal(100)
            ).intValue()+"";
            ...
            try{
                ...
                //内网穿透  （信息提示还需要调试！！！）
                param.put("notify_url", "http://s1.z100.vip:32606/yibanke-api/amect/recieveMessage");
                ...
                if(prepayId != null){
                    ...
                    if(rows!=1){
                        throw new YibankeException("更新缴费单ID失败");
                    ...
                    throw new YibankeException("创建缴费订单失败");
                }

            }catch (Exception e){
                log.error("创建缴费订单失败", e);
                throw new YibankeException("创建缴费订单失败");
            }

        }else{
            throw new YibankeException("没有找到缴费单");
        }
    }
    ...
    public void searchNativeAmectPayResult(HashMap param) {
        ...
        if (MapUtil.isNotEmpty(map)) {
            ...
            try {
                ...
                log.error("执行异常", e);
                throw new YibankeException("执行异常");
            }
        }
    }
