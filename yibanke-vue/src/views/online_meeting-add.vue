<template>
	<el-dialog title="线上课程申请" :close-on-click-modal="false" v-model="visible" width="692px">
		<el-form :model="dataForm" ref="dataForm" :rules="dataRule" label-width="60px">
			...
			<el-form-item label="日期" prop="date">
				<el-date-picker
					...
					clearable="clearable"
				></el-date-picker>
				<span class="note">课程日期只能是当前或者未来的日期，不能是以往的日期</span>
			</el-form-item>
...
export default {
	data: function() {
		return {
			...
			users: [],
			dataRule: {
				title: [{ required: true, pattern: '^[a-zA-Z0-9\u4e00-\u9fa5]{2,30}$', message: '课程主题格式错误' }],
				desc: [{ required: true, message: '课程内容为必填' }],
