<template>
	...
	<el-dialog
		...
		v-if="isAuth(['ROOT', 'STUDENT:INSERT', 'STUDENT:UPDATE'])"
	...
	>
		<el-form :model="dataForm" ref="dataForm" :rules="dataRule" label-width="80px">
			...
			<el-form-item label="入学日期" prop="hiredate">
				<el-date-picker
		...
		dataFormSubmit: function() {
		    ...
		    this.$refs['dataForm'].validate(valid => {
		        if (valid) {
					...
					//模板字符串？````  ???
		            that.$http(`user/${!that.dataForm.id ? 'insertStudent' : 'update'}`, 'POST', data, true, resp => {
		                if (resp.rows == 1) {
		                    that.$message({
		                        message: '操作成功',
		                        type: 'success',
