<template>
	...
	>
		<el-form :model="dataForm" ref="dataForm" :rules="dataRule" label-width="80px">
			...
			<el-form-item label="入校日期" prop="hiredate">
				<el-date-picker
		...
		init: function(id) {
			...
			that.$nextTick(() => {
				...
				that.$http('role/searchAllRole', 'GET', null, true, function(resp) {
					...
					/* let str = JSON.stringify(resp.list)
					let jstr = eval(str)
					jstr[4].roleName = "学办助理"
					str = JSON .stringify(jstr)
					console.log(str)
					console.log(typeof(resp.list)) */
					/* let str = JSON.stringify(resp.list)
					let jstr = eval(str)
					jstr[4].roleName = "juese"
					str = JSON .stringify(jstr)
					console.log(str)
					console.log(typeof(resp.list)) */
				...
				//dataForm.id不为0时候  为 修改；
				//修改用户业务中，还要给表单加载当前用户信息
				if (that.dataForm.id) {
		        ...
		        if (valid) {
		            ...
		            that.$http(`user/${!that.dataForm.id ? 'insert' : 'update'}`, 'POST', data, true, resp => {
		                if (resp.rows == 1) {
		                    ...
		                    that.visible = false;
							//触发自定义事件@refreshDataList进行更新页面  ====>  跳转第一页重新加载
