    ...
    <div v-if="isAuth(['ROOT', 'ROLE:SELECT'])">
        <el-form :inline="true" :model="dataForm" :rules="dataRule" ref="dataForm">
            <el-form-item prop="roleName">
                ...
                >
				<template >
						<span v-if="dataForm.roleName=='HR'">学办助理</span>
						<span v-if="dataForm.roleName=='讲师'">讲师</span>
						<span v-if="dataForm.roleName=='学生'">学生</span>
						<span v-if="dataForm.roleName=='班长'">班长</span>		
				</template>
				</el-input>
            </el-form-item>
            <el-form-item>
        ...
        >
			...
			>
			<template #default="scope">
					<span v-if="scope.row.roleName=='超级管理员'">超级管理员</span>
					<span v-if="scope.row.roleName=='HR'">学办助理</span>
					<span v-if="scope.row.roleName=='老师'">老师</span>
					<span v-if="scope.row.roleName=='学生'">学生</span>
					<span v-if="scope.row.roleName=='班长'">班长</span>		
				</template>
			</el-table-column>
            ...
            <el-table-column header-align="center" align="center" width="150" label="操作">
                <template #default="scope">
					...
					<!-- :disabled ：不具备先关权限或者是系统内置角色或者有关联用户 删除按钮禁止点击-->
                    <el-button
                        type="text"
                        size="medium"
...
export default {
    ...
    methods: {
        ...
        selectable: function(row, index) {
			...
			//users值得是分页查询时候 查询到的某个角色对应的用户数量
