<template>
	<div v-if="isAuth(['ROOT', 'USER:SELECT'])">
		...
		>
			...
			<template #default="scope">
					<span v-if="scope.row.roles=='HR'">学办助理</span>
					<span v-if="scope.row.roles=='老师'">老师</span>
					<span v-if="scope.row.roles=='学生'">学生</span>
					<span v-if="scope.row.roles=='班长'">班长</span>	
					<span v-if="scope.row.roles=='老师，超级管理员'">老师，超级管理员</span>
			</template>
			</el-table-column>
	...
	methods: {
		loadRoleList: function() {
			...
			that.$http('role/searchAllRole', 'GET', null, true, function(resp) {
				resp.list[4].roleName = "学办助理"
				resp.list[3].roleName = "测试角色"
