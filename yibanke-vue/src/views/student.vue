<template>
	<div v-if="isAuth(['ROOT', 'STUDENT:SELECT'])">
		<el-form :inline="true" :model="dataForm" :rules="dataRule" ref="dataForm">
			...
			<el-form-item>
				...
				<el-button
					size="medium"
					type="primary"
					:disabled="!isAuth(['ROOT', 'STUDENT:INSERT'])"
					@click="addHandle()"
				>
					新增
				</el-button>
				<el-button
					size="medium"
					type="danger"
					:disabled="!isAuth(['ROOT', 'STUDENT:DELETE'])"
					@click="deleteHandle()"
				>
					批量删除
				</el-button>
		...
		>
			...
			/>
			<el-table-column prop="hiredate" header-align="center" align="center" min-width="130" label="入校日期" />
			<el-table-column
				prop="roles"
				header-align="center"
				align="center"
				...
				:show-overflow-tooltip="true"
			/>
			<el-table-column prop="dept" header-align="center" align="center" min-width="120" label="班级" />
			<!-- <el-table-column prop="status" header-align="center" align="center" min-width="100" label="状态" /> -->
			<el-table-column header-align="center" align="center" width="150" label="操作">
				<template #default="scope">
					<!-- 通过scope.row获得的是当前行的用户 -->
					<el-button
						type="text"
						size="medium"
						v-if="isAuth(['ROOT', 'STUDENT:UPDATE'])"
					...
					<!-- <el-button
						...
						size="medium"
						v-if="isAuth(['ROOT', 'STUDENT:UPDATE'])"
						:disabled="scope.row.status == '离校' || scope.row.root"
						@click="dimissHandle(scope.row.id)"
					>
						离校
					</el-button> -->
					<!-- isAuth(['ROOT', 'USER:DELETE'])  保证用户有权限 否则禁用按钮 -->
					<el-button
						type="text"
						size="medium"
						:disabled="scope.row.root"
						v-if="isAuth(['ROOT', 'STUDENT:DELETE'])"
...
export default {
	...
	data() {
		return {
			...
			dataList: [],
			roleList: [],//存从数据库查询出来的数据
			deptList: [],//存从数据库查询出来的数据
			pageIndex: 1,//记录当翻页
			pageSize: 10,
			totalCount: 0,
			dataListLoading: false,
			dataListSelections: [],
			addOrUpdateVisible: false,//默认弹窗不显示
			dimissVisible: false,
			dataRule: {
	...
	methods: {
		...
		loadDataList: function() {
		    ...
		    that.$http('user/searchStudentByPage', 'POST', data, true, function(resp) {
		        let page = resp.page;
		        let list = page.list;
		        for (let one of list) {
		            if (one.status == 1) {
