<template>
	<el-dialog
		:title="!dataForm.id ? '新增缴费记录' : '修改缴费记录'"
	...
	>
		<el-form :model="dataForm" ref="dataForm" :rules="dataRule" label-width="60px">
			<el-form-item label="类型" prop="typeId">
				<el-select v-model="dataForm.typeId" placeholder="缴费类型" size="medium" style="width:40%">
					<el-option v-for="one in amectTypeList" :label="one.type" :value="one.id" />
				</el-select>
				<span class="desc">必须选择一个缴费类型，缴费金额可以自动生成</span>
			</el-form-item>
			<el-form-item label="金额" prop="amount">
				<el-input v-model="dataForm.amount" size="medium" style="width:40%" placeholder="缴费金额" clearable />
...
export default {
	data: function() {
		return {
			...
			users: [],
			dataRule: {
				typeId: [{ required: true, message: '缴费类型为必填' }],
				amount: [
					{
						required: true,
				...
				reason: [{ required: true, message: '缴费原因为必填' }],
				members: [
