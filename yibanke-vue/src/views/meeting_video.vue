	...
	<div>
		<el-row :gutter="15">
			...
			<el-col :span="19">
				...
				<p class="desc">
					会议过程中，不需要发言的会场要主动将本地语音关闭，保证会场安静
				</p>
			</el-col>
			<!-- 用户列表区域 -->
			<el-col :span="5">
						...
						<ul class="user-list">
							<li v-for="one in userList">
								...
								<div class="mic-container">
									<img class="mic-green" :id="'mic-' + one.userId"
					...
					if (!checkResult.result) {
						...
						if (that.meetingStatus) {
							...
							client.on('stream-added', event => {
								...
								that.$http('user/searchNameAndDept', 'POST', {
									id: userId
								}, true, function(resp) {
									let name = resp.name;
									let dept = resp.dept;
							...
							client
								...
								.then(() => {
									...
									//执行签到
									that.$http('meeting/updateMeetingPresent', 'POST', {
									...
									localStream.setVideoProfile('720p'); //设置分辨率
